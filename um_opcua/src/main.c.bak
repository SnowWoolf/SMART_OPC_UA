...
load_mapping_csv(const char *filename) {
    ...
    for(size_t i = 0; i < g_mappingCount; i++) {
        printf("MAP[%zu]: type=%d measure=%s api_tag=%s display=%s\n",
               i,
               g_mappings[i].device_type,
               g_mappings[i].measure,
               g_mappings[i].api_tag,
               g_mappings[i].display);
    }
    ...
    for(size_t i = 0; i < g_meterCount; i++) {
        ...
        for(size_t j = 0; j < g_mappingCount; j++) {
            ...
            if(isGetTime) {
                ...
                UA_Variant_setScalarCopy(&vAttr.value, &initDt,
                                         &UA_TYPES[UA_TYPES_DATETIME]);
            } else {
                double initValue = 0.0;
                vAttr.dataType = UA_TYPES[UA_TYPES_DOUBLE].typeId;
                UA_Variant_setScalarCopy(&vAttr.value, &initValue,
                                         &UA_TYPES[UA_TYPES_DOUBLE]);
            }
